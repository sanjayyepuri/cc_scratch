...
auto main() -> int {
  ...
  auto fd = shm_open("hello.mem", O_CREAT | O_RDWR, 0600);
  if (fd == -1) {
  ...
  auto *mem = static_cast<HelloBuffer *>(mmap(
      nullptr, sizeof(HelloBuffer), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0));
  ...
  while (true) {
    ...
    std::cout << "writer:" << mem->value << std::endl;
  }
